...
struct CreationView: View {
  ...
  var body: some View {
	 ZStack{
		...
		VStack(alignment: .leading, spacing: 15){
		  ...
		  //			MARK: Igredient section
		  IngredientsView(selectedIngredient: $vm.userIngredients, showCamera: $showCamera, leftScan: (3 - vm.user.freeScanUses))
		...
		.safeAreaInset(edge: .bottom) {
		  if !StoreManager.shared.hasFullAccess{
			 Text("Free creations left: \(max(0, 5 - vm.user.freeGenerationsUsed)) ")
  ...
  private var creationBtn: some View{
	 VStack{
		Button{
		  if !StoreManager.shared.hasFullAccess && vm.user.freeGenerationsUsed >= 5 {
			 NavigationManager.shared.popup = .weeklyLimit(.recipeGeneration)
		  }else{
			 withAnimation(.bouncy){
				if vm.ableToCreate{
				  vm.request()
